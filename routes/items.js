...
router.get('/', (req, res) => {
  const items = queryAll('SELECT * FROM items ORDER BY created_at DESC');
  res.json(items);
});

router.get('/:itemCode', (req, res) => {
  const item = queryOne('SELECT * FROM items WHERE item_code = ?', [req.params.itemCode]);
...
router.post('/', (req, res) => {
  ...
  try {
    execute('INSERT INTO items (item_code, description, default_spq) VALUES (?, ?, ?)',
...
router.put('/:itemCode', (req, res) => {
  const { description, default_spq } = req.body;
  const result = execute('UPDATE items SET description = ?, default_spq = ? WHERE item_code = ?',
...
router.delete('/:itemCode', (req, res) => {
  const reelCount = queryOne('SELECT COUNT(*) as count FROM reels WHERE item_code = ?', [req.params.itemCode]);
  if (reelCount && reelCount.count > 0) {
    return res.status(400).json({ error: `Cannot delete - ${reelCount.count} reels exist for this item` });
  }
  const result = execute('DELETE FROM items WHERE item_code = ?', [req.params.itemCode]);
  if (result.changes === 0) return res.status(404).json({ error: 'Item not found' });
