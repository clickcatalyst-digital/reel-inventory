...
router.post('/', (req, res) => {
  ...
  const item = queryOne('SELECT * FROM items WHERE item_code = ?', [item_code]);
  ...
  try {
    if (totalBoxes === 0) {
      // No box — create reels without box assignment
      for (let r = 0; r < totalReels; r++) {
        const reelNumber = getNextReelNumber();
        execute('INSERT INTO reels (reel_number, item_code, box_number, quantity, notes) VALUES (?, ?, ?, ?, ?)',
      ...
      for (let b = 0; b < totalBoxes; b++) {
        const boxNumber = getNextBoxNumber();
        const reelsInThisBox = reelsPerBox + (b < remainder ? 1 : 0);

        execute('INSERT INTO boxes (box_number, item_code, reel_count) VALUES (?, ?, ?)',
